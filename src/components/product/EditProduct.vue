...
import { editProductAPI, showProductAPI } from '../../services/products';
import useVuelidate from '@vuelidate/core'
import { required, minLength, minValue, numeric, integer } from '@vuelidate/validators'

// Se define los props, donde se obtiene el id del producto, el cual es obligatorio
...
// Se obtiene el producto a editar con el servicio de productos y el prop productId
const getProduct = () => {
    showProductAPI({
        productId: props.productId,
        funcSuccess: response => {
            formData.value.name = response.data.product.name;
            formData.value.description = response.data.product.description;
            formData.value.price = response.data.product.price;
            formData.value.stock = response.data.product.stock;
        }
    })
}
...
// Al montar el componente se obtiene la informacion del producto a editar
onMounted(() => {
   getProduct(); 
});
</script>
...
<template>
    <form class="max-w-md mx-auto" @submit.prevent="handleSubmit">
        <div class="relative z-0 w-full mb-5 group">
            <input type="text" v-model="formData.name" id="floating_name" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " />
        ...
        <div class="relative z-0 w-full mb-5 group">
            <input step="0.01" type="number" v-model="formData.price" id="floating_price" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " />
        ...
        <div class="relative z-0 w-full mb-5 group">
            <input type="number" v-model="formData.stock" id="floating_repeat_stock" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " />
        ...
        <div class="flex justify-end gap-3 mt-6">
            ...
            <router-link to="/" class="py-2.5 px-5 me-2 mb-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Cancelar</router-link>
        </div>
    </form>
</template>
